...
public class PlatformArena extends ApplicationAdapter {
	...
	public void render() {
		...
		for (Enemy e: enemies) {
			...
			for (Projectile p: projectiles) {
				if (e.hitbox.contains(new Vector2(p.x, p.y))) {
					p.destroy = true;
					e.damage(p.damage);
				}
			}
		...
		while (e.hasNext()) {
			...
			if (en.destroy) {
				...
				if (MathUtils.random(0, 1) == 1 || enemies.size < 30) enemies.add(new SeekerEnemy(800 * MathUtils.random(0, 1), 600 * MathUtils.random(0, 1)));
				if (MathUtils.random(0, 1) == 1) enemies.add(new SeekerEnemy(800 * MathUtils.random(0, 1), 600 * MathUtils.random(0, 1)));
			}
		}
	...
	public void platformCollisions() {
		// Collisions
		for (Rectangle platform : platforms) {
			if (platform.overlaps(new Rectangle(player.x, player.y, 50, 100)) 
					&& player.yMove < 0
					&& player.yLast >= platform.y
					&& player.y < platform.y) {
				player.yMove = 0;
				player.y = platform.y;
				player.onGround = true;
